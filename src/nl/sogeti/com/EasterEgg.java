...
public class EasterEgg {
    ...
    /**
     * Draws an easter egg through a modified ellipse equation: (x−a)²/rx² +
     * (y−b)²/ry² = 1
     * <p>
     * Modified ellipse equation for vertical egg:
     * (x−centerXPoint)²*1000/(horizontalRadius² * (1+(0.025 * y))) +
     * (y−centerYPoint)²*1000/verticalRadius² = 1000 x, y -> coordinates of a single
     * point on ellipse
     *
    ...
    private static void drawSingleEggPoint(final EggMetrics eggMetrics, int yCoordinate, int xCoordinate) {
        if (isXYPointInsideEgg(eggMetrics, xCoordinate, yCoordinate)) {
            System.out.print(EggDecoration.getColors(yCoordinate, xCoordinate, eggMetrics.getPhase()).getColor());
    ...
    private static boolean isXYPointInsideEgg(final EggMetrics eggMetrics, final int xCoordinate,
            final int yCoordinate) {
        return (((calculateSquareOfDistanceFromCenterXPoint(eggMetrics.getCenterXPoint(), xCoordinate)
                * EGG_EQUATION_SCALE_FACTOR)
                / (calculateSquareOfRadius(eggMetrics.getHorizontalRadius()) * factorToChangeToEggShape(yCoordinate)))
                + ((calculateSquareOfDistanceFromCenterYPoint(eggMetrics.getCenterYPoint(), yCoordinate)
                        * EGG_EQUATION_SCALE_FACTOR)
                        / calculateSquareOfRadius(eggMetrics.getVerticalRadius()))) < EGG_EQUATION_SCALE_FACTOR;
    }
